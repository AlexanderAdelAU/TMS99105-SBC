...
Write-Output "`n--- staging FILESTAT build directory ---"
...
foreach($m in @("FILEIO","FILEDATA","OVL_WORD","OVL_CHAR","OVL_RPT")) {
    Compile-Mod $m
}

# Assemble the generated A99 modules.
foreach($m in @("FILESTAT","FILEIO","FILEDATA","OVL_WORD","OVL_CHAR","OVL_RPT")) {
    Assemble $m
}

$OvlFiles = @("OVL_WORD.R99","OVL_CHAR.R99","OVL_RPT.R99")
...
foreach($a in @("O_WORD","O_CHAR","O_RPT")) {
...
$ResidentObjects = @(
    "FILESTAT.R99","FILEIO.R99","FILEDATA.R99","OVLMGR.R99","OVLAPI.R99"
)
...
Need-File ".\FILESTAT.EXE"

# Only final deliverables leave build\.
Copy-Item -LiteralPath ".\FILESTAT.EXE" -Destination (Join-Path $DistDir "FILESTAT.EXE") -Force
...
$MonitorExe = Join-Path $MonitorDir "FILESTAT.EXE"
Copy-Item -LiteralPath ".\FILESTAT.EXE" -Destination $MonitorExe -Force

Set-Location -LiteralPath $ProjectRoot

Write-Output "`nBUILD SUCCESSFUL"
Write-Output "  .\dist\FILESTAT.EXE"
Write-Output "  .\dist\SAMPLE.TXT"
Write-Output ("  deployed: " + $MonitorExe)
Write-Output "Run on the SBC: FILESTAT SAMPLE.TXT /L"
